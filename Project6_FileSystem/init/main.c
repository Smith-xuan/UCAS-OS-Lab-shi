...
#define TASKS_INFO_BLOCK_ID_ADDR 0xffffffc0502001fa
#define TASKS_INFO_lOC 0xffffffc050300000 //?
...
static void init_task_info(void)
{
    ...
    bios_sdread(kva2pa(TASKS_INFO_lOC),1,tasksInfoBlockId);
    ...
    for (int i = 0; i < TASK_MAXNUM; i++){
        ...
        tasks[i].mem_size = tasksInfoPtr[i].mem_size;
    }
...
//    pt_regs->regs[2] = (ptr_t)pt_regs;  //sp
    pt_regs->regs[2] = USER_STACK_ADDR;  //sp
    pt_regs->regs[4] = pcb;
    ...
    pt_switchto->regs[1] = USER_STACK_ADDR;//sp  
...
//           // uint64_t ENTRY_POINT = load_task_img(j);
            ...
            pcb[j].pgdir = allocPage(); // alloc a pgdir, va
            memcpy((char*)pcb[j].pgdir, (char*)pa2kva(PGDIR_PA), PAGE_SIZE); //copy the kernel page(va)
...
//    syscall[SYSCALL_THREAD_CREATE] = &thread_create;
    ...
    syscall[SYSCALL_PTHREAD_CREATE] = &pthread_create;
    syscall[SYSCALL_SHM_GET] = &shm_page_get;
    syscall[SYSCALL_SHM_DT] = &shm_page_dt;

    syscall[SYSCALL_NET_SEND] = &do_net_send;
    syscall[SYSCALL_NET_RECV] = &do_net_recv;

    syscall[SYSCALL_FS_MKFS] = &do_mkfs;
    syscall[SYSCALL_FS_STATFS] = &do_statfs;
    syscall[SYSCALL_FS_MKDIR] = &do_mkdir;
    syscall[SYSCALL_FS_CD] = &do_cd;
    syscall[SYSCALL_FS_LS] = &do_ls;
    syscall[SYSCALL_FS_RMDIR] = &do_rmdir;

    syscall[SYSCALL_FS_TOUCH] = &do_touch;   
    syscall[SYSCALL_FS_CAT] = &do_cat; 
    syscall[SYSCALL_FS_FOPEN] = &do_fopen;
    syscall[SYSCALL_FS_FREAD] = &do_fread;    
    syscall[SYSCALL_FS_FWRITE] = &do_fwrite;
    syscall[SYSCALL_FS_FCLOSE] = &do_fclose; 
    syscall[SYSCALL_FS_LN] = &do_ln;
    syscall[SYSCALL_FS_RM] = &do_rm;
    syscall[SYSCALL_FS_LSEEK] = &do_lseek;               
}

void cancel_mapping()
{
    uint64_t va;
    uint64_t pgdir = 0xffffffc051000000;
    va = 0x50200000; 
    uint64_t vpn2 = (va >> 30) & ~(~0 << 9);
    uint64_t vpn1 = (va >> 21) & ~(~0 << 9);
    PTE *pt1 = (PTE *)pgdir + vpn2;
    PTE *final_pte = (PTE *)pa2kva(get_pa(*pt1)) + vpn1;
    *pt1 = 0;
    *final_pte = 0;
...
int main(void)
{
    ...
    // master core
        ...
        //cancel the mapping of boot part
        //cancel_mapping();
        ...
        //check if there is a fs, and if not, make one
