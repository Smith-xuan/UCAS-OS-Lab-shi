...
static void screen_write_ch(char ch)
{
    if (ch == '\n')
    {
        next_running->cursor_x = 0;
        next_running->cursor_y++;
    }
    else
    {
        new_screen[SCREEN_LOC(next_running->cursor_x, next_running->cursor_y)] = ch;
        next_running->cursor_x++;
    }
}
...
void screen_clear(void)
{
    ...
    next_running->cursor_x = 0;
    next_running->cursor_y = 0;
    screen_reflush();
}

void screen_move_cursor(int x, int y)
{
    next_running->cursor_x = x;
    next_running->cursor_y = y;
...
void screen_reflush(void)
{
    ...
    vt100_move_cursor(next_running->cursor_x, next_running->cursor_y);
}
