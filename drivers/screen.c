...
static void screen_write_ch(char ch)
{
    ...
    if (ch == '\n')
    {
        next_running[cpu_id]->cursor_x = 0;
        next_running[cpu_id]->cursor_y++;
    }
    else
    {
        new_screen[SCREEN_LOC(next_running[cpu_id]->cursor_x, next_running[cpu_id]->cursor_y)] = ch;
        next_running[cpu_id]->cursor_x++;
    }
}
...
void screen_clear(void)
{
    ...
    next_running[cpu_id]->cursor_x = 0;
    next_running[cpu_id]->cursor_y = 0;
    screen_reflush();
}
...
{
    uint64_t cpu_id = get_current_cpu_id();
    next_running[cpu_id]->cursor_x = x;
    next_running[cpu_id]->cursor_y = y;
...
void screen_backspace()
{
    ...
    screen_reflush();
    next_running[cpu_id]->cursor_x--;
    vt100_move_cursor(next_running[cpu_id]->cursor_x, next_running[cpu_id]->cursor_y);
}
